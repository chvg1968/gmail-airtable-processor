...
async function getOAuth2Client(): Promise<OAuth2Client> {
  ...
  try {
    ...
    if (credentials.access_token) {
      ...
      logger.error(
        "Failed to refresh access token, no access_token in credentials. Response:",
        credentials,
      );
      throw new Error(
        "Failed to refresh access token, no access_token in credentials.",
      );
    }
  } catch (error: any) {
    logger.error(
...
export async function getEmailContent(
  ...
  try {
    ...
    logger.error(
      `Error fetching email content for message ${messageId}:`,
      error,
    );
    return null;
  }
...
export async function getGmailProfile(): Promise<string | null | undefined> {
  try {
    ...
    logger.debug(
      "Successfully connected to Gmail. User email:",
