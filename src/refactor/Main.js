    ...
    for (const msg of messages) {
      try {
        ...
        if (/help@lodgify\.com/i.test(from)) {
          Logger.log("[Main] Skip por remitente Lodgify help: from=%s subject=%s", from, subject);
          skippedCount++;
          continue;
        }
        ...
        if (nonBooking) {
          Logger.log("[Main] Skip por no-booking (soporte/seguridad). from=%s subject=%s", from, subject);
          skippedCount++;
          continue;
        }
        ...
        if (isSupportOrUpdateEmail) {
          Logger.log("[Main] Skip correo de soporte/actualización: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isNonReservationEmail && !/reservation|booking|confirmed/i.test(subject)) {
          Logger.log("[Main] Skip correo no relacionado con reservas: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isForwardedWithReservationInfo) {
          Logger.log("[Main] Skip correo reenviado con info de reserva (posible falso positivo): %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (hasReservationNumberButNotConfirmation) {
          Logger.log("[Main] Skip correo con número de reserva pero no es confirmación original: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isForwardedNonConfirmation) {
          Logger.log("[Main] Skip correo reenviado que no es confirmación: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isLodgifyNonReservation) {
          Logger.log("[Main] Skip correo de Lodgify que no es confirmación de reserva: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isReservationInfoButNotOriginal) {
          Logger.log("[Main] Skip correo con info de reserva pero no es confirmación original: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (containsReservationInfoButNotConfirmation) {
          Logger.log("[Main] Skip correo con info de reserva pero no es confirmación: %s", subject);
          skippedCount++;
          continue;
        }
        ...
        if (isLodgifyConfirmed) {
          Logger.log("[Main] Procesando confirmación real de Lodgify: %s", subject);
        ...
        const hasValidReservationData = dto.guestName && dto.checkInDate && dto.checkOutDate;
        if (!hasValidReservationData) {
          Logger.log("[Main] Skip DTO sin datos de reserva válidos: guestName='%s', checkIn='%s', checkOut='%s'", 
            dto.guestName || 'MISSING', dto.checkInDate || 'MISSING', dto.checkOutDate || 'MISSING');
          skippedCount++;
          continue;
        }
        ...
        if (processedReservations.has(duplicateKey)) {
          Logger.log("[Main] Skip duplicado detectado: %s (ya procesado en esta ejecución)", duplicateKey);
          skippedCount++;
          continue;
        }
        ...
        if (dto.reservationNumber && dto.reservationNumber.trim()) {
          ...
          if (processedReservations.has(reservationKey)) {
            Logger.log("[Main] Skip duplicado por número de reserva: %s (ya procesado en esta ejecución)", reservationKey);
            skippedCount++;
            continue;
          }
        ...
        Logger.log("[Main] Mapeo de propiedad - Platform: %s, Input: '%s'", platform, propertyInput);
        
        const mapped = PropertyService.findPropertyMapping(platform, propertyInput);
        Logger.log("[Main] Mapeo de propiedad - Resultado: %s (alias: %s)", mapped.name, mapped.aliasMatched);
        
        dto.Property = mapped.name;

        // Regla de preferencia: si es Vrbo/Lodgify y ya hay Airbnb para mismo huésped+fechas, saltar
        if (/vrbo|lodgify/i.test(String(platform))) {
          ...
          if (AirtableService.existsAirbnbForGuestDates(CONFIG, g, ci, co)) {
            Logger.log(
              "[Main] Skip Vrbo/Lodgify por preferencia Airbnb (guest+fechas): %s - %s → %s",
              g,
              ci,
              co
            );
