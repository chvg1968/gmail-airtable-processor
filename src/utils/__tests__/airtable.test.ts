...
import type { ExtractedBookingData } from '../../services/gemini';
import Airtable from 'airtable';

// Hacemos los mocks disponibles globalmente para las pruebas
declare global {
  // eslint-disable-next-line no-var
  var mockBase: any;
  var mockTable: any;
  var mockAirtableConstructor: jest.Mock;
}
...
const baseBookingData: ExtractedBookingData = {
  guestName: 'John Doe',
  platform: ['airbnb'],
...
describe('upsertBookingToAirtable', () => {
  ...
  describe('para reservas de Vrbo', () => {
    it('debe establecer NeedsDateReview como false independientemente de las fechas', async () => {
      const testData1: ExtractedBookingData = {
        ...baseBookingData,
        platform: ['vrbo'],
        bookingDate: '2024-01-01',
        checkInDate: '2025-01-01',
      };
      const testData2: ExtractedBookingData = {
        ...baseBookingData,
        platform: ['vrbo'],
        bookingDate: '2025-06-20',
        checkInDate: '2026-01-01',
      };
      const testData3: ExtractedBookingData = {
        ...baseBookingData,
        platform: ['vrbo'],
        bookingDate: undefined,
        checkInDate: '2025-12-31',
      };
      const result1 = await upsertBookingToAirtable(testData1, mockConfig, 'test-message-id');
      const result2 = await upsertBookingToAirtable(testData2, mockConfig, 'test-message-id');
      const result3 = await upsertBookingToAirtable(testData3, mockConfig, 'test-message-id');
