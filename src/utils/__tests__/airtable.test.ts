...
describe("upsertBookingToAirtable", () => {
  describe("para reservas de Airbnb", () => {
    ...
    it("debe establecer NeedsDateReview como true si la diferencia es mayor a 330 días", async () => {
      ...
      const result = await upsertBookingToAirtable(
        testData,
        mockConfig,
        "test-message-id",
      );
  ...
  describe("para reservas de Vrbo", () => {
    it("debe establecer NeedsDateReview como false independientemente de las fechas", async () => {
      ...
      const result1 = await upsertBookingToAirtable(
        testData1,
        mockConfig,
        "test-message-id",
      );
      const result2 = await upsertBookingToAirtable(
        testData2,
        mockConfig,
        "test-message-id",
      );
      const result3 = await upsertBookingToAirtable(
        testData3,
        mockConfig,
        "test-message-id",
      );
